...
public class VerilusAsteroidGenerator extends BlankPlanetGenerator{
    ...
    void asteroid(int ax, int ay, int rad){
        ...
        for(int x = ax - rad; x <= ax + rad; x++){
            for(int y = ay - rad; y <= ay + rad; y++){
                if(tiles.in(x, y) && Mathf.dst(x, y, ax, ay) < rad) { // / rad + Simplex.noise2d(seed, octaves, persistence, 1f / scale, x, y) * mag < thresh){
    ...
    public void generate(){
        ...
        //walls at insides
        pass((x, y) -> {
            if(floor == background || Ridged.noise2d(seed + 1, x, y, 4, 0.73f, 1.4f / 60f) > 0.21f || Mathf.within(x, y, sx, sy, 20 + Ridged.noise2d(seed, x, y, 3, 0.6f, 1f / 30f) * 6f) || Mathf.within(x, y, xr, yr, 20 + Ridged.noise2d(seed, x, y, 3, 0.5f, 1f / 30f) * 6f)) return;
