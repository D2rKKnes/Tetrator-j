...
public class SpeedupEnergyTurret extends PowerTurret{
  ...
  public static final Stat fireRateMaxStat = new Stat("fire-rate-max", StatCat.function);
  public static final Stat overheatCooldownStat = new Stat("overheat-cooldown", StatCat.function);
  private static final Color lightSkyBack = Color.valueOf("#8DB0FF").cpy().lerp(Color.white, 0.2f);

	public SpeedupEnergyTurret(String name){
		super(name);
	}
	...
	public void setBars(){
		...
		addBar("overheat",
				(SpeedupEnergyTurretBuild entity) -> new Bar(
						() -> Core.bundle.format("terra.bar.overheat", Strings.autoFixed(Mathf.clamp(entity.overheat / overheatTime, 0, overheatTime) * 100, 0)),
	...
	public void setStats(){
		super.setStats();
		stats.add(fireRateMaxStat, Strings.autoFixed((1 + maxSpeedupScl) * 100, 0) + "%");
		stats.add(overheatCooldownStat, overheatTime / overheatCoolAmount / Time.toSeconds, StatUnit.seconds);
	...
	public class SpeedupEnergyTurretBuild extends PowerTurretBuild{
